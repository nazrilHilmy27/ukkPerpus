...
<div class="card border-0 rounded-5 shadow-lg">
    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
        ...
        <a href="{{ route('koleksi.khusus.create') }}" class="btn btn-light btn-sm"><i class="fas fa-plus"></i>Tambah Koleksi</a>
    </div>
    <div class="card-body bg-light">
        <table id="myTable" class="display table table-hover align-middle">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $k->judul }}</td>
                    <td>{{ $k->lokasi }}</td>
                    <td><span class="">{{ $k->jenis }}</span></td>
                    @if ($k->kondisi == 'Baik')
                        <td><span class="badge bg-success p-2">{{ $k->kondisi }}</span></td>
                    @elseif ($k->kondisi == 'Rusak')
                        <td><span class="badge bg-danger p-2">{{ $k->kondisi }}</span></td>
                    @else
                        <td><span class="badge bg-warning p-2">{{ $k->kondisi }}</span></td>    
                    @endif
                    @if ($k->file_digital)
                        <td>{{ Str::limit($k->file_digital,15, '...') }}</td>
                    @else
                        <td><small class="text-muted fst-italic">Tidak ada file</small></td>
                    @endif
                    <td>
                        <button type="button" class="btn btn-outline-success btn-sm" data-bs-toggle="modal"
                            data-bs-target="#staticBackdrop{{ $k->id }}"><i class="fas fa-eye"></i></button>
                        <a href="{{ route('koleksi.khusus.edit', $k->id) }}" class="btn btn-outline-warning btn-sm"><i class="fas fa-edit"></i></a>
                        <form action="{{ route('koleksi.khusus.delete', $k->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus koleksi ini')" class="d-inline">
                            @csrf
                            @method('DELETE')
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        let sidebar = document.querySelectorAll('.sidebar .nav-link');
    ...
    $(document).ready(function () {
        $('#myTable').DataTable({
            ...
            language: {
                search: "Cari:",
                lengthMenu: "Menampilkan _MENU_data ",
